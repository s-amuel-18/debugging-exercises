...
function returnBook(loan, returnDate) {
  // CORREGIDO: asignar la fecha real de devolución, no loan.dueDate
...
function calculateFine(loan, finePerDay) {
  ...
  // CORREGIDO: diffDays > 0 significa devolución tardía
...
function renewLoan(loan, extraDays) {
  // CORREGIDO: solo renovar si returnDate === null (préstamo todavía activo)
...
function getAvailableBooks(books) {
  // CORREGIDO: usar === para comparar, no = (que sería asignación)
...
function getMemberLoans(loans, memberId) {
  // CORREGIDO: comparar loan.member.id con memberId, no el objeto completo
...
function getMostBorrowedBooks(loans, n) {
  ...
  return Object.values(counts)
    // CORREGIDO: comparador numérico, no booleano
