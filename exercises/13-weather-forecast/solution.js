...
async function fetchMultipleCities(cities) {
  // CORREGIDO: Se agrega await para esperar a que Promise.all resuelva todos los resultados
...
async function getHottestCity(cities) {
  // CORREGIDO: Se agrega await para obtener el array resuelto, no una Promise
...
async function getAverageTemperature(cities) {
  const weatherData = await fetchMultipleCities(cities);
  // CORREGIDO: Se agrega await antes de fetchMultipleCities (ya corregido arriba)
  // y se divide entre weatherData.length (no entre cities.length, aunque son iguales,
  // para operar sobre los datos reales resueltos)
